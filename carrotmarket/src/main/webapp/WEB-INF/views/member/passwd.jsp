...
<section id="content">
	<section id="user-profile">
		<div class="d-flex justify-content-evenly mb-3" style="border-bottom: 1px #e9ecef solid;">
			...
			<button type="button" class="myInfo_Btn" onclick="location.href='passwd'" style="color : #ff6f0f;">비밀번호 변경</button>
			<button type="button" class="myInfo_Btn" onclick="location.href='remove'">회원탈퇴</button>
		</div>
		
		<form action="passwd" style = "padding-top: 30px; width: 80%; margin : 0 auto;" id="join_form" method="post">	
			<h1>기존 비밀번호</h1>
			  <input type="password" name="passwd" required oninvalid="this.setCustomValidity('기존 비밀번호를 입력하세요.')" oninput="this.setCustomValidity('')" />
			...
			<span id="id_check_result"></span>
			 ...
			 <h1>새 비밀번호</h1>
	         <input type="password" name="newPasswd" required oninvalid="this.setCustomValidity('새 비밀번호를 입력하세요.')" oninput="this.setCustomValidity('')" />
